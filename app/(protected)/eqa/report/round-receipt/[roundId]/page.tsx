...
export default async function EqaRoundReceiptReportPage({ params, searchParams }: { params: Promise<{ roundId: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  const sampleCodes = [...new Set(round.results.map((result) => result.sampleCode).filter((code): code is string => Boolean(code)))]
  ...
  return <EqaReportFrame backHref={eqaBackHref(tab)} code={EQA_DOCUMENT_CODES['round-receipt']} orientation="portrait" draft={round.documentState.status !== 'approved'}>
    ...
    <table className="receipt-table"><tbody>
      ...
      <tr><th>ระบุการเก็บตัวอย่าง</th><td><Box checked={round.storageCondition === 'refrigerated'} /> แช่เย็นที่อุณหภูมิ {round.storageTemperatureC ?? '.......'} °C</td><td><Box checked={round.storageCondition === 'room'} /> เก็บที่อุณหภูมิห้อง</td><td><Box checked={round.storageCondition === 'other'} /> อื่นๆ {round.storageNote ?? '........................'}</td></tr>
      <tr><th>ชนิดของตัวอย่าง</th><td colSpan={3}>{round.specimenType ?? ''}</td></tr><tr><th>รหัสตัวอย่าง</th><td colSpan={3}>{sampleCodes.join(', ')}</td></tr><tr><th>ผู้บันทึกการรับตัวอย่าง</th><td colSpan={3}>{round.receiverName ?? ''}</td></tr><tr><th>ผู้ทำการตรวจวิเคราะห์</th><td colSpan={3}>{round.analystName ?? ''}</td></tr><tr><th>วันที่ตรวจวิเคราะห์</th><td colSpan={3}>{date(round.analysisDate)}</td></tr><tr><th>วันที่ส่งผลการตรวจวิเคราะห์</th><td colSpan={3}>{date(round.submissionDate)}</td></tr><tr><th>วิธีส่งผลการตรวจวิเคราะห์</th><td colSpan={3}>{round.submissionMethod ?? ''}</td></tr><tr className="details"><th>รายละเอียดอื่นๆ (ถ้ามี)</th><td colSpan={3}>{round.otherDetails ?? '-'}</td></tr>
    </tbody></table>
    <section className="analyst-review"><div className="review-heading"><h2>บันทึกการตรวจสอบโดยผู้ทำการตรวจวิเคราะห์</h2><div /></div><div className="review-grid"><div className="result-lines">{round.results.map((result) => <p key={result.id}>{result.sampleCode ?? '-'}&nbsp;&nbsp;·&nbsp;&nbsp;{result.analyte}&nbsp;&nbsp;-&nbsp;&nbsp;{result.submittedValue ?? '-'} {result.unit ?? ''}{result.ctValue == null ? '' : `  - Ct ${result.ctValue}`}</p>)}{!round.results.length ? <p>&nbsp;</p> : null}</div><div className="review-sign"><div className="mini-line">{analystApproval?.signatureAttachmentId ? <img src={`/api/attachments/${analystApproval.signatureAttachmentId}`} alt="ลายเซ็นผู้ทำการตรวจวิเคราะห์" className="review-signature" /> : analystApproval ? 'รอลงนามดิจิทัล' : ''}</div><p>{analystApproval?.signerName ?? round.analystName ?? ''}</p><p>(ผู้ทำการตรวจวิเคราะห์)</p><p>วันที่ {approvalDate(analystApproval?.approvedAt) || '........................'}</p></div></div></section>
    <section className="manager-review"><div className="review-heading"><h2>การตรวจสอบและทบทวนโดยผู้จัดการวิชาการ</h2><div /></div><div className="review-grid"><div className="reviewed-text">{managerApproval ? 'ตรวจสอบแล้ว' : ''}</div><div className="review-sign"><div className="mini-line">{managerApproval?.signatureAttachmentId ? <img src={`/api/attachments/${managerApproval.signatureAttachmentId}`} alt="ลายเซ็นผู้จัดการวิชาการ" className="review-signature" /> : managerApproval ? 'รอลงนามดิจิทัล' : ''}</div><p>{managerApproval?.signerName ?? ''}</p><p>(ผู้จัดการวิชาการ)</p><p>วันที่ {approvalDate(managerApproval?.approvedAt) || '........................'}</p></div></div></section>
    <EqaApprovalGrid roles={['analyst', 'technical-manager']} approvals={round.approvals} />
    <style>{`
      .receipt-title{margin:0 0 10px;text-align:center;font-size:23px}.receipt-table{width:100%;table-layout:fixed;font-size:18px}.receipt-table th,.receipt-table td{border:1px solid #111;padding:5px 7px;vertical-align:middle}.receipt-table th{width:33%;text-align:center;font-weight:700}.receipt-table td{height:34px}.receipt-table .details td,.receipt-table .details th{height:65px}.choice-box{position:relative;display:inline-block;width:15px;height:15px;margin:0 5px 0 0;border:1.4px solid #111;vertical-align:-1px}.choice-box.checked:after{content:"";position:absolute;left:3px;top:-1px;width:6px;height:11px;border-right:2px solid #111;border-bottom:2px solid #111;transform:rotate(42deg)}.analyst-review,.manager-review{border:1px solid #111;border-top:0}.review-heading,.review-grid{display:grid;grid-template-columns:2fr 1fr}.review-heading h2{margin:0;padding:4px 6px;font-size:19px}.review-heading>div{border-left:1px solid #111}.review-grid{min-height:120px}.result-lines,.reviewed-text{padding:8px 20px;text-align:center}.result-lines p{margin:6px}.review-sign{display:flex;flex-direction:column;justify-content:center;border-left:1px solid #111;padding:10px;text-align:center}.review-sign p{margin:4px}.mini-line{display:flex;min-height:45px;align-items:center;justify-content:center;border-bottom:1px dotted #111}.review-signature{display:block;max-width:95%;max-height:40px;object-fit:contain}.manager-review .review-grid{min-height:105px}.reviewed-text{display:flex;align-items:center;justify-content:center;font-size:20px}.receipt-table+.analyst-review{margin-top:0}.eqa-report-page .approval-grid{display:none}
    `}</style>
  </EqaReportFrame>
}

...
const schema = z.object({ isActive: z.boolean().optional(), providerId: z.string().uuid().optional(), name: z.string().trim().min(1).max(200).optional(), code: z.string().trim().max(80).nullable().optional(), analyteScope: z.string().trim().max(2000).nullable().optional(), roundsPerYear: z.number().int().positive().nullable().optional() })

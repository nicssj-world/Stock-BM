...
export const equipmentSchema = z.object({
  ...
  serialNumber: z.string().trim().max(160).nullable().optional(), assetNumber: z.string().trim().max(160).nullable().optional(), locationId: z.string().uuid().nullable().optional(), installedOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().optional(), warrantyUntil: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().optional(),

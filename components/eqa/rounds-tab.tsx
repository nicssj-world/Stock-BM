...
function RoundCard({ round, data, actor, focus, onNavigate, onOk, onErr, forceCollapsed }: { round: EqaRound; data: EqaWorkspace; actor: BmActor; focus?: EqaFocus | null; onNavigate: (target: EqaReadinessTarget) => void; onOk: Update; onErr: (text: string) => void; forceCollapsed?: { signal: number; value: boolean } }) {
  ...
  return <Card className="p-4">
    ...
    {!editingResult && canAddResults ? <form onSubmit={saveResult} className="mt-3 grid items-end gap-2 md:grid-cols-4 xl:grid-cols-10"><Field label="Sample code"><Input value={resultForm.sampleCode} onChange={(event) => setResultForm({ ...resultForm, sampleCode: event.target.value })} /></Field><Field label="Analyte">{analyteOptions.length ? <Select value={resultForm.analyte} onChange={(event) => setResultForm({ ...resultForm, analyte: event.target.value })} required><option value="">— เลือกรายการทดสอบ —</option>{analyteOptions.map((analyte) => <option key={analyte} value={analyte}>{analyte}</option>)}</Select> : <Input value={resultForm.analyte} onChange={(event) => setResultForm({ ...resultForm, analyte: event.target.value })} required />}</Field><Field label="ผลที่ส่ง"><Input value={resultForm.submittedValue} onChange={(event) => setResultForm({ ...resultForm, submittedValue: event.target.value })} /></Field><Field label="Unit"><Input value={resultForm.unit} onChange={(event) => setResultForm({ ...resultForm, unit: event.target.value })} /></Field><Field label="เชื่อม IQC panel"><Select value={resultForm.iqcAnalyteId} onChange={(event) => setResultForm({ ...resultForm, iqcAnalyteId: event.target.value })}><option value="">— ไม่ใช้ Six Sigma —</option>{iqcPanels.map((panel) => <option key={panel.name} value={panel.id}>{panel.name}</option>)}</Select></Field><Field label="Ct"><Input type="number" step="any" value={resultForm.ctValue} onChange={(event) => setResultForm({ ...resultForm, ctValue: event.target.value })} /></Field><div className="flex gap-1"><Button disabled={busy}>+ ผลที่ส่ง</Button></div></form> : null}

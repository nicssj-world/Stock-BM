...
type SetupKey = 'equipment' | 'lot' | 'baseline' | 'plan' | 'advanced'
...
function isSetupKey(value: string | null | undefined): value is SetupKey {
  return value === 'equipment' || value === 'lot' || value === 'baseline' || value === 'plan' || value === 'advanced'
}
...
function defaultSetupHealth(data: IqcWorkspace) {
  return {
    ...
    readyCount: [data.instruments.length, data.controlLots.length, data.controlPlans.length].filter(Boolean).length,
    attentionCount: 1 + (data.controlPlans.length ? 0 : 1) + 1,
    blockedCount: Number(!data.instruments.length) + Number(!data.controlLots.length),
  }
}
...
export function IqcSettingsCenter({ data, actor, initialSetup, initialInstrumentId, initialLotId, initialAnalyteId, onOk, onErr }: IqcSettingsCenterProps) {
  ...
  const vlAnalytes = useMemo(() => data.analytes.filter((analyte) => /-VL\b/i.test(analyte.code)), [data.analytes])
  const [taskKey, setTaskKey] = useState<SetupKey>(() => isSetupKey(initialSetup) ? initialSetup : firstIncomplete)
  const [instrumentId, setInstrumentId] = useState(() => initialInstrumentId || data.instruments[0]?.id || '')
  const [lotId, setLotId] = useState(() => initialLotId || data.controlLots.find((lot) => lot.isActive)?.id || data.controlLots[0]?.id || '')
  const [analyteId, setAnalyteId] = useState(() => initialAnalyteId || vlAnalytes[0]?.id || '')
  ...
  return (
    <div className="space-y-4">
      ...
      <div id="iqc-setup-panel" role="region" aria-live="polite">
        ...
        {taskKey === 'baseline' ? (
          <BaselineTask data={data} actor={actor} instrumentId={instrumentId} lotId={lotId} analyteId={analyteId} vlAnalytes={vlAnalytes} onInstrumentChange={setInstrumentId} onSelect={selectBaseline} onOk={onOk} onErr={onErr} />
        ) : null}
...
function BaselineTask({ data, actor, instrumentId, lotId, analyteId, vlAnalytes, onInstrumentChange, onSelect, onOk, onErr }: { data: IqcWorkspace; actor: BmActor; instrumentId: string; lotId: string; analyteId: string; vlAnalytes: IqcWorkspace['analytes']; onInstrumentChange: (value: string) => void; onSelect: (lotId: string, analyteId: string, instrumentId?: string) => void; onOk: (text: string, data: IqcWorkspace) => void; onErr: (text: string) => void }) {
  ...
  const matrixRows = activeLots.flatMap((lot) => vlAnalytes.map((analyte) => {
  ...
  const selected = Boolean(instrumentId && data.controlLots.find((lot) => lot.id === lotId)?.isActive && vlAnalytes.some((analyte) => analyte.id === analyteId))
  return (
    <div className="space-y-4">
      <Card className="space-y-4 p-5">
        ...
        <div className="grid gap-3 md:grid-cols-3"><Field label="เครื่องมือ" hint="baseline แยกตามเครื่องมือ"><Select value={instrumentId} onChange={(event) => onInstrumentChange(event.target.value)}><option value="">เลือกเครื่องมือ</option>{data.instruments.filter((instrument) => instrument.isActive).map((instrument) => <option key={instrument.id} value={instrument.id}>{instrument.code} · {instrument.name}</option>)}</Select></Field><Field label="Control lot" hint="เลือก lot ที่ต้องการทบทวน"><Select value={lotId} onChange={(event) => onSelect(event.target.value, analyteId)}><option value="">เลือก Control lot</option>{activeLots.map((lot) => <option key={lot.id} value={lot.id}>{lot.controlMaterialName}{lot.level ? ` · ${lot.level}` : ''} · {lot.lotNumber}</option>)}</Select></Field><Field label="Analyte / ระดับ" hint="VL ทุก assay/ระดับใช้ policy เดียวกัน แต่ baseline แยก scope"><Select value={analyteId} onChange={(event) => onSelect(lotId, event.target.value)}><option value="">เลือก analyte</option>{vlAnalytes.map((analyte) => <option key={analyte.id} value={analyte.id}>{analyte.code} · {analyte.dataType === 'qualitative' ? 'Normal' : 'quantitative'}</option>)}</Select></Field></div>
        <p className="rounded-md border border-[#d8e7e7] bg-[#f8fbfb] px-3 py-2 text-xs text-[#58747d]">Preview: จะทบทวน {matrixRows.length} assay/ระดับที่มีผลในเครื่องมือที่เลือก · ค่าที่ไม่ถึง 20 ผลจะยังไม่สามารถ activate quantitative baseline</p>
      </Card>
      <Card className="overflow-hidden p-0"><div className="flex items-center justify-between border-b border-[#e1ebec] px-5 py-4"><div><h4 className="font-bold text-[#173d50]">VL baseline matrix</h4><p className="mt-1 text-xs text-[#789097]">เลือก “ทบทวน” เพื่อดูผลรายครั้งและผลกระทบก่อนบันทึก</p></div><span className="mono text-xs text-[#789097]">{matrixRows.length} rows</span></div><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-xs"><thead className="bg-[#f7fbfb] text-[#789097]"><tr><th className="px-4 py-3">Assay / ระดับ</th><th className="px-4 py-3">Control lot</th><th className="px-4 py-3">CoA</th><th className="px-4 py-3">Candidate</th><th className="px-4 py-3">สถานะ baseline</th><th className="px-4 py-3" /></tr></thead><tbody className="divide-y divide-[#edf2f2] text-[#3f5c64]">{matrixRows.map(({ lot, analyte, spec, baseline, candidateN }) => <tr key={`${lot.id}:${analyte.id}`} className={lot.id === lotId && analyte.id === analyteId ? 'bg-[#f1faf9]' : ''}><td className="px-4 py-3"><span className="mono font-semibold text-[#173d50]">{analyte.code}</span><span className="ml-2 text-[#789097]">{analyte.dataType === 'qualitative' ? 'Normal' : lot.level ?? '—'}</span></td><td className="px-4 py-3">{lot.lotNumber}</td><td className="px-4 py-3">{spec?.manufacturerLower != null && spec.manufacturerUpper != null ? <span className="mono">{fmt(spec.manufacturerLower)}–{fmt(spec.manufacturerUpper)}</span> : <span className="text-[#a0b0b4]">ไม่มี CoA</span>}</td><td className="px-4 py-3 mono">{candidateN}</td><td className="px-4 py-3">{baseline ? <StatusBadge tone={baseline.baselineType === 'observed_seed' ? 'investigate' : 'accepted'} label={baseline.baselineType === 'observed_seed' ? `Observed seed v${baseline.version}` : `Approved v${baseline.version}`} /> : candidateN >= (analyte.dataType === 'qualitative' ? 1 : 20) ? <StatusBadge tone="investigate" label="พร้อมทบทวน" /> : <StatusBadge tone="not_evaluated" label="ข้อมูลยังไม่ครบ" />}</td><td className="px-4 py-3 text-right"><Button type="button" variant="secondary" className="min-h-11" onClick={() => onSelect(lot.id, analyte.id)}>{lot.id === lotId && analyte.id === analyteId ? 'กำลังเลือก' : 'ทบทวน'}</Button></td></tr>)}</tbody></table></div></Card>
      {selected ? <BaselineReviewPanel key={`${lotId}:${analyteId}:${instrumentId}:${data.baselines?.find((baseline) => baseline.controlLotId === lotId && baseline.analyteId === analyteId && baseline.instrumentId === instrumentId && baseline.state === 'approved')?.id ?? 'none'}`} actor={actor} controlLotId={lotId} analyteId={analyteId} instrumentId={instrumentId} onOk={onOk} onErr={onErr} /> : <Notice tone="warning">เลือกเครื่องมือ, Control lot และ analyte เพื่อเปิด Baseline Review</Notice>}
    </div>
  )
}
...
function ControlPlanTask({ data, onOk, onErr }: { data: IqcWorkspace; onOk: (text: string, data: IqcWorkspace) => void; onErr: (text: string) => void }) {
  const vlAnalytes = data.analytes.filter((analyte) => /-VL\b/i.test(analyte.code))
  const testSets = [...new Set(vlAnalytes.flatMap((analyte) => parseTestSets(analyte.groupLabel)))].sort()
  const [instrumentId, setInstrumentId] = useState(data.instruments[0]?.id ?? '')
  const [testSet, setTestSet] = useState(testSets[0] ?? '')
  const [frequency, setFrequency] = useState<'daily' | 'per-run'>('daily')
  const [rules, setRules] = useState<string[]>(ALL_RULES)
  const [busy, setBusy] = useState(false)
  const selectedAnalytes = vlAnalytes.filter((analyte) => !testSet || parseTestSets(analyte.groupLabel).includes(testSet))
  ...
  async function save() {
    ...
    if (!window.confirm(`ยืนยันตั้งค่า policy vl-standard-v1 ให้ ${selectedAnalytes.length} analytes และ ${requiredLevels.length} levels หรือไม่`)) return
    setBusy(true)
    try {
      const result = await api<{ iqc: IqcWorkspace }>('/api/iqc/control-plans', { method: 'POST', body: JSON.stringify({ analyteIds: selectedAnalytes.map((analyte) => analyte.id), instrumentId, requiredLevels, frequency, westgardRules: rules, policyProfile: 'vl-standard-v1', isActive: true }) })
      onOk('บันทึกกำหนดการรันและ policy VL แล้ว', result.iqc)
  ...
  return (
    <Card className="space-y-5 p-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Field label="เครื่องมือ"><Select value={instrumentId} onChange={(event) => setInstrumentId(event.target.value)}><option value="">เลือกเครื่องมือ</option>{data.instruments.filter((instrument) => instrument.isActive).map((instrument) => <option key={instrument.id} value={instrument.id}>{instrument.code} · {instrument.name}</option>)}</Select></Field>
        <Field label="Test set"><Select value={testSet} onChange={(event) => setTestSet(event.target.value)}><option value="">ทุก VL test set</option>{testSets.map((set) => <option key={set} value={set}>{set}</option>)}</Select></Field>
      ...
      <div className="rounded-md border border-[#c8e2df] bg-[#f5fbfa] px-4 py-3 text-sm text-[#176b68]"><strong>Preview:</strong> จะประเมิน {selectedAnalytes.length} analytes และ {requiredLevels.length} levels · policy <span className="mono">vl-standard-v1</span></div>
      ...
      <div className="space-y-3">
        <div>
          <p className="font-semibold text-[#173d50]">กติกาเริ่มต้นสำหรับ VL</p>
          <p className="mt-1 text-xs text-[#789097]">ระบบใช้ policy มาตรฐาน: แจ้งเตือนที่ 1-2s, reject ที่ 1-3s/2-2s และเปิด investigation เมื่อพบ R4s/4-1s/10x</p>
        </div>
        ...
        <details className="rounded-md border border-dashed border-[#cfdee0] bg-[#fbfdfd] p-3">
          ...
          <p className="mt-2 text-xs text-[#789097]">ใช้เมื่อ SOP ของห้องปฏิบัติการกำหนด rule ต่างจาก preset; การประเมินยังคงใช้ policy VL และ R4s เฉพาะผลต่างระดับใน run เดียวกัน</p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#e1ebec] pt-4"><span className="text-xs text-[#789097]">Bulk apply จะมีผลกับ {selectedAnalytes.length} analytes · CD4 ใช้ <span className="mono">cd4-legacy</span> ต่อไป</span><Button disabled={busy} onClick={() => void save()}>{busy ? 'กำลังบันทึก…' : 'บันทึกและไปขั้นถัดไป'}</Button></div>
    </Card>
  )
}
